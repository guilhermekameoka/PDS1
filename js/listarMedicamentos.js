document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function carregarMedicamentos() {
    try {
      ...
      medicamentosContainer.innerHTML = '<p class="text-center text-gray-500 my-4">Carregando medicamentos...</p>';
      
      // Faz a requisição para a API
      const response = await fetch(`http://localhost:3000/medicamento/usuario/${usuarioId}`);
      ...
      if (medicamentos.length === 0) {
        medicamentosContainer.innerHTML = '<p class="text-center text-gray-500 my-4">Nenhum medicamento registrado</p>';
        return;
      }
      ...
      medicamentos.forEach(medicamento => {
        ...
        medicamentoElement.className = "p-4 bg-gray-100 rounded-lg shadow-md medicamento-item";
      ...
      medicamentosContainer.innerHTML = '<p class="text-center text-red-500 my-4">Erro ao carregar medicamentos. Tente novamente mais tarde.</p>';
    }
  }
