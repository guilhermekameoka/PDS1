document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function carregarConsultas() {
    try {
      ...
      const response = await fetch(`http://localhost:3000/consulta/paciente/${idosoId}`);
        ...
        consultasData.forEach(consulta => {
          ...
          consultaElement.innerHTML = `
            <div class="flex-1">
              ...
              ${consulta.observacoes ? `<p class="text-xs text-gray-500 mt-1">${consulta.observacoes}</p>` : ''}
            </div>
